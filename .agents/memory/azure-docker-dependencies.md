...
**Why:** Both `npm ci` and `npm install`, across multiple npm 10 versions and with audit/funding disabled, reproducibly crashed after about 72 seconds with npm's internal `Exit handler never called!` error. Changing concurrency and separating build stages did not help. The pnpm-based container build was confirmed working.
